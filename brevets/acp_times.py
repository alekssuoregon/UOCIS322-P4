...
def _calculate_arrival(start_time, distance):
    open_delta_hours = 0
    open_delta_minutes = 0
    closed_delta_hours = 0
    closed_delta_minutes = 0
    ...
    while not done:
        ...
        min_km_per_min = rules[1] / 60
        max_km_per_min = rules[2] / 60

        open_delta = stretch_dist / max_km_per_min
        closed_delta = stretch_dist / min_km_per_min

        open_delta_hours += int(open_delta / 60)
        open_delta_minutes += int(round(open_delta % 60, 0))

        closed_delta_hours += int(closed_delta / 60)
        closed_delta_minutes += int(round(closed_delta % 60, 0))
        i+=1
